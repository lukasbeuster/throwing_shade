...
def main(osmid, dates):
    ...
    # -------------------------------
    # Set the path to your folder containing the 2010 files.
    folder_path = "../data/clean_data/solar/271110/"

    # Regular expression to match and extract p_xxx in filenames for year 2010.
    pattern = re.compile(r"p_(\d+)_2010")

    # Initialize a set to store unique p_xxx strings.
    p_values_2010 = set()

    # Loop through all files in the folder to create the exclusion list.
    ...
    # Convert the set to a sorted list for further usage.
    ...
    for date in dates:
        ...
        polygons = process_shade_metrics(osmid, timestr, polygons, p_values_2010_list)

    # Save the updated polygons after processing all dates
    output_path = f"../results/output/{osmid}/{osmid}_sidewalks_with_stats_multiple_dates_filtered.gpkg"
...
def process_shade_metrics(osmid, timestr, polygons, p_values_2010_list):
    """
    ...
    for bldg_tree in ['building', 'tree']:
        ...
        # Find raster files
        raster_files = find_raster_files(root_directory, f"{timestr}.tif", p_values_2010_list)
        if not raster_files:
        ...
        polygons = process_daily_stats(polygons, root_directory, timestr, bldg_tree, p_values_2010_list)
        ...
        polygons = process_hourly_statistics_parallel(polygons, root_directory, bldg_tree, timestr, p_values_2010_list)


    return polygons

def process_daily_stats(polygons, root_directory, timestr, bldg_tree, p_values_2010_list):
    """
...
def process_hourly_statistics(polygons, root_directory, bldg_tree, timestr, p_values_2010_list):
    """
    ...
    for time in available_times:
        ...
        hour_files = find_raster_files(root_directory, timestamp, p_values_2010_list)
        if not hour_files:
...
def process_hourly_statistics_parallel(polygons, root_directory, bldg_tree, timestr, p_values_2010_list):
    """
...
# -------------------------------------------
# Step 2: Modified find_raster_files function
# -------------------------------------------
def find_raster_files(root_dir, file_pattern, exclude_substrings=None):
    """
    Find raster files matching a specific pattern in a directory,
    excluding files that contain any of the specified substrings.

    Parameters:
        root_dir (str): Directory in which to search.
        file_pattern (str): A substring that must be present in matching file names.
        exclude_substrings (list, optional): A list of substrings. Any file containing one of these substrings will be excluded.

    Returns:
        list: A list of matching file paths that do not contain any of the exclude_substrings.
    """
    ...
    for root, _, files in os.walk(root_dir):
        for f in files:
            # Check if the file has the required pattern.
            if file_pattern in f:
                # Exclude the file if any substring from the exclusion list is found in the file name.
                if not any(exclude in f for exclude in exclude_substrings):
